...
declare module "*.m.css" {
	...
	// I trust the build process and the tsserver plugin.
	// Set to `any` instead of `{ [key: string]: string }` because I
	// turned on the `noUncheckedIndexedAccess` ts compiler option.
...
interface Array<T> {
	freeze(): ReadonlyArray<T>;
	seal(): SealedArray<T>;
}
interface SealedArray<T> extends Omit<Array<T>, "push"|"pop"|"shift"|"unshift"|"splice">, Iterable<T> {}
...
declare namespace TU {
	...
	type DeepRo<T> =
		...
		: DeepRoObj<T>;
	interface DeepRoArr<T> extends ReadonlyArray<DeepRo<T>> {}
	type DeepRoObj<T> = { readonly [P in keyof T]: DeepRo<T[P]> };
}

/**
 * See the WebPack config's DefinePlugin.
 */
declare namespace DEF {
	...
	/**
	 * Asserts that developers working on this project did not break
	 * any of the internal specifications.
	 *
	 * Wherever used, please add a "Enforced By: _" comment.
	 */
	const DevAssert: boolean;
